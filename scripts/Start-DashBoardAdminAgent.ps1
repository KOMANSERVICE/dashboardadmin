...
## ⚠️ DEPLACEMENT DES CARTES - OBLIGATOIRE

**REGLE ABSOLUE:** Tu DOIS TOUJOURS deplacer l'issue a la fin de l'analyse.

### Commandes de deplacement (CASE-INSENSITIVE):
``````powershell
# Si analyse VALIDE -> deplacer vers "Todo"
gh project item-edit --project-id PROJECT_ID --id ITEM_ID --field-id STATUS_FIELD_ID --single-select-option-id "Todo"

# Si analyse BLOQUEE -> deplacer vers "AnalyseBlock"
gh project item-edit --project-id PROJECT_ID --id ITEM_ID --field-id STATUS_FIELD_ID --single-select-option-id "AnalyseBlock"
``````

### Utiliser cette fonction PowerShell:
``````powershell
# Fonction a executer OBLIGATOIREMENT
function Move-ToColumn {
    param([int]\$IssueNumber, [string]\$Column)
    ...
    \$item = \$projectData.data.repository.issue.projectItems.nodes | 
        Where-Object { \$_.project.number -eq $ProjectNumber } | 
        Select-Object -First 1
    
    if (\$item) {
        # Obtenir le field Status
        ...
        \$statusField = \$fields.data.node.field
        \$targetOption = \$statusField.options | Where-Object { \$_.name -eq \$Column }
        ...
        if (\$targetOption) {
            ...
            '
            Write-Host "[OK] Issue deplacee vers \$Column" -ForegroundColor Green
        }
    }
}

# APPELER OBLIGATOIREMENT a la fin:
Move-ToColumn -IssueNumber $IssueNumber -Column "Todo"      # Si valide
# OU
Move-ToColumn -IssueNumber $IssueNumber -Column "AnalyseBlock"  # Si bloquee
``````
...
``````powershell
# Fonction a utiliser pour deplacer
gh api graphql -f query='
    mutation {
        updateProjectV2ItemFieldValue(input: {
